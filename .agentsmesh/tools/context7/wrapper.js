...
const noXSS = (s) => !XSS.some((p) => p.test(s));
...
function reputationFromTrust(trust) {
    if (trust === undefined)
        return undefined;
    if (trust >= 8)
        return "High";
...
export const resolveLibraryId = {
    ...
    description: "Resolve a library name to Context7-compatible library IDs (keyless; optional CONTEXT7_API_KEY raises rate limits).",
    auth: [],
    wraps: { type: "rest" },
    input: resolveInput,
 ...
 */
function deriveLibraryName(libraryId) {
...
export const getLibraryDocs = {
    ...
    description: "Fetch up-to-date documentation for a Context7 library ID (keyless; optional CONTEXT7_API_KEY raises rate limits).",
    auth: [],
    wraps: { type: "rest" },
    input: docsInput,
    output: docsOutput,

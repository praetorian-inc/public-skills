...
const validateNoPathTraversal = (s) => !PATH_TRAVERSAL.some((p) => p.test(s));
const validateNoCommandInjection = (s) => !COMMAND_INJECTION.some((p) => p.test(s));
const validateNoControlChars = (s) => !CONTROL_CHARS.test(s);
...
function safeOptional(describe) {
    return z
        .string()
 ...
 */
async function fbRequest(apiKey, method, path, opts) {
...
export const listPosts = {
    ...
    handler: async (args, ctx) => {
        const query = { limit: args.limit ?? 10, sortBy: args.sortBy ?? "createdAt" };
        if (args.cursor)
            query.cursor = args.cursor;
        if (args.categoryId)
            query.categoryId = args.categoryId;
        if (args.statusId)
            query.statusId = args.statusId;
        if (args.status)
        ...
        if (args.q)
            query.q = args.q;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/posts", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
export const getPost = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", `v2/posts/${encodeURIComponent(args.postId)}`);
        if (!res.ok) {
            throw new Error(res.status === 404 ? `Post not found: ${args.postId}` : `Failed to get post: ${res.message}`);
        }
        const post = res.data;
        const postData = {
            id: post.id,
...
export const createPost = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/posts", {
            json: {
                title: args.title,
        ...
        if (!res.ok)
            throw new Error(`Failed to create post: ${res.message}`);
        const post = res.data;
        const postData = {
            id: post.id,
            slug: post.slug,
...
export const updatePost = {
    ...
    handler: async (args, ctx) => {
        ...
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "put", `v2/posts/${encodeURIComponent(args.postId)}`, { json: updateData });
        if (!res.ok)
            throw new Error(`Failed to update post: ${res.message}`);
        const post = res.data;
        const postData = {
            id: post.id,
            slug: post.slug,
...
export const deletePost = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", `v2/posts/${encodeURIComponent(args.postId)}`);
        if (!res.ok)
            throw new Error(`Failed to delete post: ${res.message}`);
...
async function fbPatch(apiKey, path, opts) {
    const headers = { "X-API-Key": apiKey };
...
export const listChangelog = {
    ...
    handler: async (args, ctx) => {
        const query = { limit: args.limit ?? 10 };
        if (args.cursor)
            query.cursor = args.cursor;
        if (args.tags)
            query.tags = args.tags.join(",");
        if (args.q)
            query.q = args.q;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/changelogs", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
export const getChangelog = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", `v2/changelogs/${encodeURIComponent(args.changelogId)}`);
        if (!res.ok) {
...
export const createChangelog = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/changelogs", {
            json: {
                title: args.title,
        ...
        if (!res.ok)
            throw new Error(`Failed to create changelog: ${res.message}`);
        const entry = res.data;
        const entryData = {
            id: entry.id,
            title: entry.title,
...
export const updateChangelog = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.tags)
            updateData.tags = args.tags;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "put", `v2/changelogs/${encodeURIComponent(args.changelogId)}`, { json: updateData });
        if (!res.ok)
            throw new Error(`Failed to update changelog: ${res.message}`);
        const entry = res.data;
        const entryData = {
            id: entry.id,
            title: entry.title,
...
export const deleteChangelog = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", `v2/changelogs/${encodeURIComponent(args.changelogId)}`);
        if (!res.ok)
            throw new Error(`Failed to delete changelog: ${res.message}`);
...
export const listCollections = {
    ...
    handler: async (args, ctx) => {
        const query = { limit: args.limit ?? 20 };
        if (args.cursor)
            query.cursor = args.cursor;
        if (args.parentId)
            query.parentId = args.parentId;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/help_center/collections", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
export const createCollection = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/help_center/collections", {
            json: {
                name: args.name,
        ...
        if (!res.ok)
            throw new Error(`Failed to create collection: ${res.message}`);
...
export const updateCollection = {
    ...
    handler: async (args, ctx) => {
        const res = await fbPatch(ctx.secrets.FEATUREBASE_API_KEY, "v2/help_center/collections/" + encodeURIComponent(args.collectionId), {
            json: {
        ...
        if (!res.ok)
            throw new Error(`Failed to update collection: ${res.message}`);
        const collection = res.data;
        const collectionData = {
            id: collection.id,
            name: collection.name,
...
export const deleteCollection = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", "v2/help_center/collections/" + encodeURIComponent(args.collectionId));
        if (!res.ok)
            throw new Error(`Failed to delete collection: ${res.message}`);
...
export const listArticles = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.q)
            query.q = args.q;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/help_center/articles", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
export const getArticle = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", `v2/help_center/articles/${encodeURIComponent(args.articleId)}`);
        if (!res.ok) {
            throw new Error(res.status === 404
...
export const createArticle = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/help_center/articles", {
            json: {
                title: args.title,
        ...
        if (!res.ok)
            throw new Error(`Failed to create article: ${res.message}`);
        const article = res.data;
        const articleData = {
            id: article.id,
            title: article.title,
...
export const updateArticle = {
    ...
    handler: async (args, ctx) => {
        const res = await fbPatch(ctx.secrets.FEATUREBASE_API_KEY, `v2/help_center/articles/${encodeURIComponent(args.articleId)}`, {
            json: {
        ...
        if (!res.ok)
            throw new Error(`Failed to update article: ${res.message}`);
        const article = res.data;
        const articleData = {
            id: article.id,
            title: article.title,
...
export const deleteArticle = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", `v2/help_center/articles/${encodeURIComponent(args.articleId)}`);
        if (!res.ok)
            throw new Error(`Failed to delete article: ${res.message}`);
...
export const listComments = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.submissionId)
            queryParams.push(`submissionId=${encodeURIComponent(args.submissionId)}`);
        if (args.changelogId)
            queryParams.push(`changelogId=${encodeURIComponent(args.changelogId)}`);
        queryParams.push(`limit=${limit}`);
        queryParams.push(`page=${page}`);
        queryParams.push(`sortBy=${sortBy}`);
        if (args.includePrivate !== undefined)
            queryParams.push(`includePrivate=${args.includePrivate}`);
        if (args.includeDeleted !== undefined)
            queryParams.push(`includeDeleted=${args.includeDeleted}`);
        const queryString = queryParams.join("&");
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", `v2/comment?${queryString}`);
        if (!res.ok)
            throw new Error(`FeatureBase Comments API error: ${res.message}`);
        return {
...
export const createComment = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.submissionId)
            form.submissionId = args.submissionId;
        if (args.changelogId)
            form.changelogId = args.changelogId;
        if (args.parentCommentId)
            form.parentCommentId = args.parentCommentId;
        if (args.isPrivate !== undefined)
            form.isPrivate = args.isPrivate;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/comment", { form });
        if (!res.ok)
            throw new Error(`Failed to create comment: ${res.message}`);
...
export const updateComment = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.isPinned !== undefined)
            form.isPinned = args.isPinned;
        if (args.isPrivate !== undefined)
            form.isPrivate = args.isPrivate;
        const res = await fbPatch(ctx.secrets.FEATUREBASE_API_KEY, `v2/comment/${encodeURIComponent(args.commentId)}`, { form });
        if (!res.ok)
            throw new Error(`Failed to update comment: ${res.message}`);
        const comment = res.data;
        return {
            id: comment.id,
            submissionId: comment.submissionId,
...
export const deleteComment = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", `v2/comment/${encodeURIComponent(args.commentId)}`);
        if (!res.ok)
            throw new Error(`Failed to delete comment: ${res.message}`);
        const deletionType = res.data.deletionType === "soft" ? "soft" : "hard";
        const resultData = { success: true, commentId: args.commentId, deletionType: deletionType };
...
export const identifyUser = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/contacts", {
            json: {
        ...
        if (!res.ok)
            throw new Error(`Failed to identify user: ${res.message}`);
...
export const getUser = {
    ...
    handler: async (args, ctx) => {
        ...
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", `v2/organization/identifyUser?${params.toString()}`);
        if (!res.ok) {
            throw new Error(res.status === 404
...
export const listUsers = {
    ...
    handler: async (args, ctx) => {
        ...
        const query = { limit, sortBy };
        if (args.cursor)
            query.cursor = args.cursor;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/contacts", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
        return {
...
export const deleteUser = {
    ...
    handler: async (args, ctx) => {
        ...
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", `v2/organization/deleteUser?${queryString}`);
        if (!res.ok)
            throw new Error(`Failed to delete user: ${res.message}`);
...
export const listCompanies = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.cursor)
            query.cursor = args.cursor;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/companies", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
export const createCompany = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/companies", {
            json: {
                name: args.name,
        ...
        if (!res.ok)
            throw new Error(`Failed to create company: ${res.message}`);
...
export const listWebhooks = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.cursor)
            query.cursor = args.cursor;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/webhooks", { query });
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
...
const createWebhookInput = z.object({
    ...
    topics: z
        ...
        .describe('Event topics to subscribe to (e.g. ["post.created", "post.voted", "post.updated", "post.deleted", "comment.created", "changelog.published"])'),
...
export const createWebhook = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/webhooks", {
            json: {
                name: args.name,
                url: args.url,
                topics: args.topics,
        ...
        if (!res.ok)
            throw new Error(`Failed to create webhook: ${res.message}`);
...
export const deleteWebhook = {
    ...
    handler: async (args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "delete", "v2/webhooks/" + encodeURIComponent(args.webhookId));
        if (!res.ok)
            throw new Error(`Failed to delete webhook: ${res.message}`);
...
export const listCustomFields = {
    ...
    handler: async (_args, ctx) => {
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/custom_fields");
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
        return {
...
export const createCustomField = {
    ...
    handler: async (args, ctx) => {
        ...
        if (args.options !== undefined)
            requestBody.options = args.options;
        if (args.min !== undefined)
            requestBody.min = args.min;
        if (args.max !== undefined)
            requestBody.max = args.max;
        if (args.itemType !== undefined)
            requestBody.itemType = args.itemType;
        if (args.required !== undefined)
            requestBody.required = args.required;
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "post", "v2/custom_fields", { json: requestBody });
        if (!res.ok)
            throw new Error(`Failed to create custom field: ${res.message}`);
        return {
...
export const listBoards = {
    ...
    handler: async (_args, ctx) => {
        ...
        const res = await fbRequest(ctx.secrets.FEATUREBASE_API_KEY, "get", "v2/boards");
        if (!res.ok)
            throw new Error(`FeatureBase API error: ${res.message}`);
